...
open class RouteStep: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        drivingSide = .right //try container.decode(DrivingSide.self, forKey: .drivingSide)
        ...
        if let instruction = try? maneuver.decode(String.self, forKey: .instruction) {
            ...
            instructions = RouteStep.getInstruction(from: maneuverType, direction: maneuverDirection, exitIndex: exitIndex, name: name)// "\(maneuverType) \(maneuverDirection?.rawValue ?? "")"
        }
    }
    private class func getInstruction(from type: ManeuverType, direction: ManeuverDirection?, exitIndex: Int?, name: String?) -> String {
        var text = ""
        switch type {
        case .depart:
            text = "Bắt đầu"
        case .arrive:
            text = "Bạn đã đến điểm đích"
        case .takeRoundabout, .turnAtRoundabout:
            text = "Đi vào bùng binh"
            if let exit = exitIndex {
                text += " và đi tiếp ở lối ra thứ \(exit)"
            }
            if let name = name, !name.isEmpty {
                text += " để vào \(name)"
            }
        case .exitRoundabout, .exitRotary:
            if let exit = exitIndex, exit != 0 {
                text += "Ra khỏi bùng binh ở lối ra thứ \(exit)"
            }
            if let name = name, !name.isEmpty {
                text += " để vào \(name)"
            }
