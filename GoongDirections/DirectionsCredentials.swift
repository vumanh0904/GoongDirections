...
public struct DirectionsCredentials: Equatable {
    ...
    /**
     The host to reach. defaults to `rsapi.goong.io`.
     */
    public let host: URL
    ...
    public init(accessToken token: String? = nil, host: URL? = nil) {
        ...
        precondition(accessToken != nil && !accessToken!.isEmpty, "A Goong access token is required. Get one at <https://account.goong.io/>")
        ...
        if let host = host {
            ...
            self.host = URL(string: "https://rsapi.goong.io")!
        }
    }
}
